    ...
    <x-filament::dropdown teleport placement="bottom-end">
        <x-slot name="trigger">
            <button type="button"
                ...
                <x-filament::icon icon="heroicon-m-chevron-down" icon-alias="panels::panel-switch-simple-icon"
        ...
        <x-filament::dropdown.list>
            @foreach ($panels as $id => $url)
                <x-filament::dropdown.list.item :href="$url" :badge="str($labels[$id] ?? $id)
                    ->substr(0, 2)
                    ->upper()" tag="a">
...
@else
    ...
    <x-filament::icon-button x-data="{}" icon="heroicon-s-square-2-stack"
        icon-alias="panels::panel-switch-modern-icon" icon-size="lg"
        @click="$dispatch('open-modal', { id: 'panel-switch' })" :label="$heading" @class(['bg-gray-100 !rounded-full dark:bg-custom-500/20'])
        style="{{ \Filament\Support\get_color_css_variables('primary', shades: [100, 500]) }}; min-width: 36px;" />

    <x-filament::modal id="panel-switch" :width="$modalWidth" alignment="center" :slide-over="$isSlideOver" :sticky-header="$isSlideOver"
        :heading="$heading" display-classes="block" class="panel-switch-modal">
        <div class="flex flex-wrap items-center justify-center gap-4 md:gap-6">
            @foreach ($panels as $id => $url)
                <a href="{{ $url }}" title="{{ $labels[$id] ?? str($id)->ucfirst() }}"
                    ...
                    <div @class([
                        ...
                        'group-hover:ring-2 group-hover:ring-primary-600' =>
                            $id !== $currentPanel->getId(),
                        'ring-2 ring-primary-600' => $id === $currentPanel->getId(),
                    ])>
                        @if ($renderIconAsImage)
                            <img class="rounded-lg panel-switch-card-image"
                    ...
                    <span @class([
                        ...
                        'text-gray-400 dark:text-gray-200 group-hover:text-primary-600 group-hover:dark:text-primary-400' =>
                            $id !== $currentPanel->getId(),
